%  Main program for the metabolic network reaction prediction experiments
%
...
%         trainr(:,1:validnumber) = [];
        ...
        for output_predS = 1:0     % whether to output the predicted reaction names and the recovered S + dS matrix
        matched_reactions = MUnames(logical(sum(match)))    %output the matched reactions
        tmp = sUS(:,check_repeat);
        sS = Model.S;
        predS = full([sS(:,perm(testnumber+1:end)),tmp(:,logical(Lambda'))]);    % predicted full S + dS matrix
        %predS = [trainr,unrepeat_US(:,logical(Lambda'))];    %the recovered S + dS matrix (binary)
        end
