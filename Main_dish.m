% Predict missing dishes (hyperlinks are recipes on materials).
% The dish names are in Chinese. 

slCharacterEncoding('UTF-8')

% Change experimental settings here.
method = 'CMM'
u = 1000;
nmissing = 400;
dataname = 'chuancai.txt';  % Sichuan recipe
%dataname = 'yuecai.txt';  % Cantonese recipe
...
%% do hyperlink prediction
[Lambda, scores] = HLpredict(S, U, nmissing, method, 1, Ulabels);
nnz(Lambda'.*Ulabels)  % show number of true positive
nmissing^2/(u+nmissing)  % compare with thoeretical random guess number
